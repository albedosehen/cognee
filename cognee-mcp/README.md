...
- `SSE_CORS_ALLOWED_ORIGINS`: Comma-separated list of allowed origins for SSE transport (default: `http://localhost:3000`)
- `HTTP_CORS_ALLOWED_ORIGINS`: Comma-separated list of allowed origins for HTTP transport (default: `http://localhost:3000`)
...
docker run \
  -e TRANSPORT_MODE=sse \
  -e SSE_CORS_ALLOWED_ORIGINS=http://localhost:3000 \
  --env-file ./.env \
  -p 8000:8000 \
  --rm -it cognee/cognee-mcp:main
...
docker run \
  -e TRANSPORT_MODE=http \
  -e HTTP_CORS_ALLOWED_ORIGINS=http://localhost:3000,http://localhost:3001 \
  --env-file ./.env \
  -p 8000:8000 \
  --rm -it cognee/cognee-mcp:main
```
...
When running the MCP server (directly with Python or via Docker), you can configure CORS origins:
...
```shell
# Single origin (direct Python usage)
SSE_CORS_ALLOWED_ORIGINS=http://localhost:3000 python src/server.py --transport sse

# Multiple origins (direct Python usage)
HTTP_CORS_ALLOWED_ORIGINS=http://localhost:3000,http://localhost:3001 python src/server.py --transport http

# Docker usage with custom CORS origins
docker run \
  -e TRANSPORT_MODE=sse \
  -e SSE_CORS_ALLOWED_ORIGINS=http://localhost:3000,https://example.com \
  --env-file ./.env \
  -p 8000:8000 \
  --rm -it cognee/cognee-mcp:main
```
